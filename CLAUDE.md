...
- **IA (`lib/bot-ia.ts`):** chama uma API compatível com OpenAI. Provedor/modelo/chave parametrizados por env (`IA_PROVIDER`/`IA_API_KEY`/`IA_MODEL`/`IA_API_URL`; presets groq/nvidia/openai). Default de código: Groq `llama-3.3-70b-versatile`; produção pode sobrescrever via secret. **`IA_PROVIDER=off` desliga a IA** sem apagar a chave (bots 100% heurística) — é o estado atual de produção. Saídas do não-espia que citam palavra do nome do evento/local são descartadas em código (`vazouNomeEvento`) e caem na heurística.
- **Heurística (`lib/bot-heuristica.ts` + `lib/bot-lexico.ts` + `lib/bot-conversa.ts`):** decisão **offline** por casamento de palavras-chave contra um léxico por evento (~15-19 termos cada). Espelha as assinaturas da IA (inclui `{confianca}`), então os mesmos limiares de `bot-agir` valem para ambas. Peças principais:
  - **Pesos por exclusividade** (`pesoTermo`): termo exclusivo vale 1, compartilhado cai até 0.25 — flexões contam como uma família ("pastor"/"pastores").
  - **Acusação por aderência + contradição:** suspeito = quem menos adere ao evento verdadeiro; fala que adere a OUTRO evento (espia chutando errado) pesa mais que silêncio.
  - **Conversa por ângulo** (`bot-conversa.ts`): a pergunta recebida é classificada (visão/pessoas/sentimento/perigo/ação/lugar/memória) e a resposta usa molde daquele ângulo + termo do cenário, flexionado por `comFormas` (artigo por gênero/número, contrações de/em, acentos, nomes próprios). Perguntas giram de ângulo por destinatário, sem repetir texto na rodada; respostas nunca repetem detalhe já dito pelo próprio bot.
  - **Discrição:** palavra pública e respostas preferem termos compartilhados; `NAO_CITAVEIS` barra verbos/números soltos (agramaticais nos moldes); `quaseObvio` barra primos morfológicos do nome ("crucificar" na Crucificação).
  - **Espia:** com palpite forte (score/margem pesados) responde com termos do evento deduzido para se misturar; sem palpite, cita um termo "seguro" (`TERMOS_SEGUROS`, comum a 3+ eventos) — evasivo sem ser vazio.
- **Sorteio (`lib/bot.ts`):** pools fixos, último recurso.

> **Gotcha ao auditar transcrições:** a heurística e o sorteio foram feitos para "soar engajados". Com IA ligada, o Groq free é gargalado por rate limit — sob carga retorna HTTP 429 e a decisão cai para a heurística/sorteio. Sintoma: "bots genéricos". Suspeite de 429 **antes** de culpar o prompt. Respostas da heurística usam moldes por ângulo da pergunta + termo do léxico, ex.: *"Senti um aperto quando vi a funda."* — seguem o tema da pergunta, mas o texto é de molde fixo (não é linguagem livre).
>
