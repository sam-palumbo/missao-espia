...
When the `IA_API_KEY` secret is set on the edge function, bot decisions are made by an LLM via an OpenAI-compatible API (`supabase/functions/game/lib/bot-ia.ts`, JSON mode). The provider is parameterized: `IA_PROVIDER` selects a preset (`nvidia` default → `deepseek-ai/deepseek-v4-pro`, `groq`, `openai`), with `IA_MODEL` and `IA_API_URL` to override the model/endpoint: each bot receives its role (the event+location only if it is not the spy), the round words, and the question/answer/vote history, and decides its word, question + target, answer, vote, accusation target, and spy guess. Timing gates (when to accuse/guess) remain random in `bot-agir.ts`. On any AI failure — or without the key — decisions fall back to the random pools in `lib/bot.ts`, so the game never stalls.
